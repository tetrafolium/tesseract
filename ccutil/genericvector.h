...
public:
  GenericVector() { init(kDefaultVectorSize); }
  ...
  int size_reserved() const { return size_reserved_; }

  int length() const { return size_used_; }

  // Return true if empty.
  bool empty() const { return size_used_ == 0; }
  ...
  // get_id, ...)
  void
  set_compare_callback(TessResultCallback2<bool, T const &, T const &> *cb);
...
template <typename T> bool cmp_eq(T const &t1, T const &t2) { return t1 == t2; }

// Used by sort()
// return < 0 if t1 < t2
// return 0 if t1 == t2
// return > 0 if t1 > t2
template <typename T> int sort_cmp(const void *t1, const void *t2) {
...
// return 0 if t1 == t2
// return > 0 if t1 > t2
template <typename T> int sort_ptr_cmp(const void *t1, const void *t2) {
...
template <typename T> class PointerVector : public GenericVector<T *> {
public:
...
template <typename T> class GenericVectorEqEq : public GenericVector<T> {
public:
  GenericVectorEqEq() {
...
template <typename T> void GenericVector<T>::init(int size) {
  size_used_ = 0;
  size_reserved_ = 0;
  data_ = 0;
  clear_cb_ = 0;
...
template <typename T> GenericVector<T>::~GenericVector() { clear(); }

// Reserve some memory. If the internal array contains elements, they are
// copied.
template <typename T> void GenericVector<T>::reserve(int size) {
...
template <typename T> void GenericVector<T>::double_the_size() {
  if (size_reserved_ == 0) {
...
// Resizes to size and sets all values to t.
template <typename T> void GenericVector<T>::init_to_size(int size, T t) {
  reserve(size);
  size_used_ = size;
...
// Return the object from an index.
template <typename T> T &GenericVector<T>::get(int index) const {
  ASSERT_HOST(index >= 0 && index < size_used_);
  return data_[index];
}

template <typename T> T &GenericVector<T>::operator[](int index) const {
  assert(index >= 0 && index < size_used_);
  return data_[index];
}

template <typename T> T &GenericVector<T>::back() const {
  ASSERT_HOST(size_used_ > 0);
  return data_[size_used_ - 1];
}
// Returns the last object and removes it.
template <typename T> T GenericVector<T>::pop_back() {
  ASSERT_HOST(size_used_ > 0);
  return data_[--size_used_];
}

// Return the object from an index.
template <typename T> void GenericVector<T>::set(T t, int index) {
  ASSERT_HOST(index >= 0 && index < size_used_);
  data_[index] = t;
}
...
template <typename T> void GenericVector<T>::insert(T t, int index) {
  ASSERT_HOST(index >= 0 && index <= size_used_);
...
// Removes an element at the given index and
// shifts the remaining elements to the left.
template <typename T> void GenericVector<T>::remove(int index) {
  ASSERT_HOST(index >= 0 && index < size_used_);
...
template <typename T> T GenericVector<T>::contains_index(int index) const {
  return index >= 0 && index < size_used_;
}

// Return the index of the T object.
template <typename T> int GenericVector<T>::get_index(T object) const {
  for (int i = 0; i < size_used_; ++i) {
...
// Return true if T is in the array
template <typename T> bool GenericVector<T>::contains(T object) const {
  return get_index(object) != -1;
}

// Add an element in the array
template <typename T> int GenericVector<T>::push_back(T object) {
  int index = 0;
...
template <typename T> int GenericVector<T>::push_back_new(T object) {
...
template <typename T> int GenericVector<T>::push_front(T object) {
  if (size_used_ == size_reserved_)
    double_the_size();
...
template <typename T> void GenericVector<T>::operator+=(T t) { push_back(t); }
...
template <typename T> void GenericVector<T>::clear() {
  if (size_reserved_ > 0) {
    if (clear_cb_ != NULL)
      for (int i = 0; i < size_used_; ++i)
...
template <typename T> void GenericVector<T>::delete_data_pointers() {
  for (int i = 0; i < size_used_; ++i)
    if (data_[i]) {
      delete data_[i];
    }
...
// Writes a vector of simple types to the given file. Assumes that bitwise
// read/write of T will work. Returns false in case of error.
template <typename T> bool GenericVector<T>::Serialize(FILE *fp) const {
...
// Returns false in case of error.
// If swap is true, assumes a big/little-endian swap is needed.
template <typename T> bool GenericVector<T>::DeSerialize(bool swap, FILE *fp) {
  inT32 reserved;
  if (fread(&reserved, sizeof(reserved), 1, fp) != 1)
    return false;
  if (swap)
...
template <typename T> bool GenericVector<T>::DeSerialize(tesseract::TFile *fp) {
  inT32 reserved;
...
// bool T::Serialize(FILE* fp) const that returns false in case of error.
// Returns false in case of error.
template <typename T> bool GenericVector<T>::SerializeClasses(FILE *fp) const {
  if (fwrite(&size_used_, sizeof(size_used_), 1, fp) != 1)
    return false;
  for (int i = 0; i < size_used_; ++i) {
...
// its argument, and finally invalidates its argument.
template <typename T> void GenericVector<T>::move(GenericVector<T> *from) {
  this->clear();
...
template <typename T> void GenericVector<T>::sort() {
