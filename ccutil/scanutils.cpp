...
// Helper functions ------------------------------------------------------------
inline size_t LongBit() { return CHAR_BIT * sizeof(long); }
...
uintmax_t streamtoumax(FILE *s, int base) {
  ...
  for (c = fgetc(s); isspace(static_cast<unsigned char>(c)) && (c != EOF);
       c = fgetc(s)) {
  }
...
double streamtofloat(FILE *s) {
  ...
  double f =
      static_cast<double>(v) + static_cast<double>(w) / static_cast<double>(k);
  if (c == 'e' || c == 'E') {
    c = fgetc(s);
    int expsign = 1;
    if (c == '-' || c == '+') {
