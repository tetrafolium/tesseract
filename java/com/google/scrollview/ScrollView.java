...
public class ScrollView {
  ...
  private static void IOLoop() {
    ...
    try {
      while (!socket.isClosed() && !socket.isInputShutdown() &&
             !socket.isOutputShutdown() && socket.isConnected() &&
             socket.isBound()) {
  ...
  private static void parseArguments(String argList, ArrayList<Integer> intList,
  ...
  private static void processInput(String inputLine) {
    ...
    if (inputLine.charAt(0) == 'w') {
      ...
      parseArguments(inputLine.substring(start + 1, end), intList, floatList,
                     stringList, boolList);
      ...
      if (colon > 1 && colon < start) {
        ...
        if (func.equals("drawLine")) {
          ...
          windows.get(windowID).textAttributes(
              stringList.get(0), intList.get(0), boolList.get(0),
              boolList.get(1), boolList.get(2));
        } else if (func.equals("drawText")) {
          windows.get(windowID).drawText(intList.get(0), intList.get(1),
                                         stringList.get(0));
        } else if (func.equals("addMenuBarItem")) {
          if (boolList.size() > 0) {
            windows.get(windowID).addMenuBarItem(
                stringList.get(0), stringList.get(1), intList.get(0),
                boolList.get(0));
          ...
          if (stringList.size() == 4) {
            windows.get(windowID).addPopupMenuItem(
                stringList.get(0), stringList.get(1), intList.get(0),
          ...
          windows.get(windowID).drawImage(image, intList.get(0),
                                          intList.get(1));
        ...
        if (idStrs[1].startsWith("= luajava.newInstance")) {
          ...
          windows.set(windowID, new SVWindow(stringList.get(1), intList.get(0),
                                             intList.get(1), intList.get(2),
                                             intList.get(3), intList.get(4),
                                             intList.get(5), intList.get(6)));
        }
        // else for forward compatibility purposes, silently ignore any
        // unrecognized function call.
      }
