...
protected:
  Dawg(DawgType type, const STRING &lang, PermuterType perm, int debug_level)
      : type_(type), lang_(lang), perm_(perm), unicharset_size_(0),
  ...
  inline UNICHAR_ID
  unichar_id_from_edge_rec(const EDGE_RECORD &edge_rec) const {
  ...
  bool match_words(WERD_CHOICE *word, inT32 index, NODE_REF node,
...
struct DawgPosition {
  ...
  DawgPosition(int dawg_idx, EDGE_REF dawgref, int punc_idx, EDGE_REF puncref,
               bool backtopunc)
      : dawg_index(dawg_idx), dawg_ref(dawgref), punc_index(punc_idx),
        punc_ref(puncref), back_to_punc(backtopunc) {}
...
public:
  ...
  SquishedDawg(EDGE_ARRAY edges, int num_edges, DawgType type,
               ...
               int debug_level)
      : Dawg(type, lang, perm, debug_level), edges_(edges),
