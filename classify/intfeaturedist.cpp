...
IntFeatureDist::IntFeatureDist()
    : size_(0), total_feature_weight_(0.0), feature_map_(NULL), features_(NULL),
...
// Set feature vector.
double
IntFeatureDist::FeatureDistance(const GenericVector<int> &features) const {
...
// Set feature vector.
double
IntFeatureDist::DebugFeatureDistance(const GenericVector<int> &features) const {
  int num_test_features = features.size();
  double denominator = total_feature_weight_ + num_test_features;
  double misses = denominator;
  for (int i = 0; i < num_test_features; ++i) {
