...
static const uinT32 kCountTable[LOOKUPTABLESIZE] = {
    MINSAMPLES, 200, 400, 600, 800, 1000, 1500, 2000}; // number of samples
...
inT32 MergeClusters(inT16 N, register PARAM_DESC ParamDesc[], register inT32 n1,
                    register inT32 n2, register FLOAT32 m[],
                    register FLOAT32 m1[], register FLOAT32 m2[]);

void ComputePrototypes(CLUSTERER *Clusterer, CLUSTERCONFIG *Config);

PROTOTYPE *MakePrototype(CLUSTERER *Clusterer, CLUSTERCONFIG *Config,
                         CLUSTER *Cluster);

PROTOTYPE *MakeDegenerateProto(uinT16 N, CLUSTER *Cluster,
                               STATISTICS *Statistics, PROTOSTYLE Style,
                               inT32 MinSamples);

PROTOTYPE *TestEllipticalProto(CLUSTERER *Clusterer, CLUSTERCONFIG *Config,
                               CLUSTER *Cluster, STATISTICS *Statistics);

PROTOTYPE *MakeSphericalProto(CLUSTERER *Clusterer, CLUSTER *Cluster,
                              STATISTICS *Statistics, BUCKETS *Buckets);

PROTOTYPE *MakeEllipticalProto(CLUSTERER *Clusterer, CLUSTER *Cluster,
                               STATISTICS *Statistics, BUCKETS *Buckets);

PROTOTYPE *MakeMixedProto(CLUSTERER *Clusterer, CLUSTER *Cluster,
                          STATISTICS *Statistics, BUCKETS *NormalBuckets,
                          FLOAT64 Confidence);
...
STATISTICS *ComputeStatistics(inT16 N, PARAM_DESC ParamDesc[],
                              CLUSTER *Cluster);

PROTOTYPE *NewSphericalProto(uinT16 N, CLUSTER *Cluster,
                             STATISTICS *Statistics);

PROTOTYPE *NewEllipticalProto(inT16 N, CLUSTER *Cluster,
                              STATISTICS *Statistics);
...
BOOL8 Independent(PARAM_DESC ParamDesc[], inT16 N, FLOAT32 *CoVariance,
                  FLOAT32 Independence);

BUCKETS *GetBuckets(CLUSTERER *clusterer, DISTRIBUTION Distribution,
                    uinT32 SampleCount, FLOAT64 Confidence);

BUCKETS *MakeBuckets(DISTRIBUTION Distribution, uinT32 SampleCount,
                     FLOAT64 Confidence);
...
void FillBuckets(BUCKETS *Buckets, CLUSTER *Cluster, uinT16 Dim,
                 PARAM_DESC *ParamDesc, FLOAT32 Mean, FLOAT32 StdDev);

uinT16 NormalBucket(PARAM_DESC *ParamDesc, FLOAT32 x, FLOAT32 Mean,
                    FLOAT32 StdDev);

uinT16 UniformBucket(PARAM_DESC *ParamDesc, FLOAT32 x, FLOAT32 Mean,
                     FLOAT32 StdDev);
...
FLOAT64 Solve(SOLVEFUNC Function, void *FunctionParams, FLOAT64 InitialGuess,
              FLOAT64 Accuracy);

FLOAT64 ChiArea(CHISTRUCT *ChiParams, FLOAT64 x);

BOOL8 MultipleCharSamples(CLUSTERER *Clusterer, CLUSTER *Cluster,
                          FLOAT32 MaxIllegal);
...
CLUSTERER *MakeClusterer(inT16 SampleSize, const PARAM_DESC ParamDesc[]) {
  ...
  Clusterer->ParamDesc = (PARAM_DESC *)Emalloc(SampleSize * sizeof(PARAM_DESC));
...
SAMPLE *MakeSample(CLUSTERER *Clusterer, const FLOAT32 *Feature, inT32 CharID) {
  ...
  Sample = (SAMPLE *)Emalloc(sizeof(SAMPLE) +
                             (Clusterer->SampleSize - 1) * sizeof(FLOAT32));
...
void CreateClusterTree(CLUSTERER *Clusterer) {
  ...
  context.candidates =
      (TEMPCLUSTER *)Emalloc(Clusterer->NumberOfSamples * sizeof(TEMPCLUSTER));
  context.next = 0;
  ...
  while (context.heap->Pop(&HeapEntry)) {
    ...
    // then we must find a new nearest neighbor
      ...
      PotentialCluster->Neighbor = FindNearestNeighbor(
          context.tree, PotentialCluster->Cluster, &HeapEntry.key);
...
void MakePotentialClusters(ClusteringContext *context, CLUSTER *Cluster,
  ...
  context->candidates[next].Neighbor = FindNearestNeighbor(
      context->tree, context->candidates[next].Cluster, &HeapEntry.key);
...
PROTOTYPE *MakePrototype(CLUSTERER *Clusterer, CLUSTERCONFIG *Config,
  ...
  Buckets =
      GetBuckets(Clusterer, normal, Cluster->SampleCount, Config->Confidence);
...
PROTOTYPE *MakeDegenerateProto( // this was MinSample
    uinT16 N, CLUSTER *Cluster, STATISTICS *Statistics, PROTOSTYLE Style,
    inT32 MinSamples) {
...
PROTOTYPE *TestEllipticalProto(CLUSTERER *Clusterer, CLUSTERCONFIG *Config,
  ...
  double F =
      Tsq * (TotalDims - EssentialN - 1) / ((TotalDims - 2) * EssentialN);
  int Fx = EssentialN;
  if (Fx > FTABLE_X)
    Fx = FTABLE_X;
  --Fx;
...
PROTOTYPE *MakeSphericalProto(CLUSTERER *Clusterer, CLUSTER *Cluster,
  ...
  for (i = 0; i < Clusterer->SampleSize; i++) {
    ...
    FillBuckets(Buckets, Cluster, i, &(Clusterer->ParamDesc[i]),
                Cluster->Mean[i], sqrt((FLOAT64)(Statistics->AvgVariance)));
...
PROTOTYPE *MakeEllipticalProto(CLUSTERER *Clusterer, CLUSTER *Cluster,
  ...
  for (i = 0; i < Clusterer->SampleSize; i++) {
    ...
    FillBuckets(
        Buckets, Cluster, i, &(Clusterer->ParamDesc[i]), Cluster->Mean[i],
        sqrt((FLOAT64)Statistics->CoVariance[i * (Clusterer->SampleSize + 1)]));
    if (!DistributionOK(Buckets))
      break;
  }
  // if all dimensions matched a normal distribution, make a proto
...
PROTOTYPE *MakeMixedProto(CLUSTERER *Clusterer, CLUSTER *Cluster,
  ...
  for (i = 0; i < Clusterer->SampleSize; i++) {
    ...
    FillBuckets(NormalBuckets, Cluster, i, &(Clusterer->ParamDesc[i]),
                Proto->Mean[i], sqrt((FLOAT64)Proto->Variance.Elliptical[i]));
...
void MakeDimUniform(uinT16 i, PROTOTYPE *Proto, STATISTICS *Statistics) {
  ...
  Proto->Mean[i] =
      Proto->Cluster->Mean[i] + (Statistics->Min[i] + Statistics->Max[i]) / 2;
...
STATISTICS *ComputeStatistics(inT16 N, PARAM_DESC ParamDesc[],
  ...
  Statistics->AvgVariance =
      (float)pow((double)Statistics->AvgVariance, 1.0 / N);
...
PROTOTYPE *NewSphericalProto(uinT16 N, CLUSTER *Cluster,
  ...
  Proto->TotalMagnitude =
      (float)pow((double)Proto->Magnitude.Spherical, (double)N);
 ...
 */
BOOL8
Independent(PARAM_DESC ParamDesc[], inT16 N, FLOAT32 *CoVariance,
            FLOAT32 Independence) {
  int i, j;
...
BUCKETS *MakeBuckets(DISTRIBUTION Distribution, uinT32 SampleCount,
                     FLOAT64 Confidence) {
  const DENSITYFUNC DensityFunction[] = {NormalDensity, UniformDensity,
                                         UniformDensity};
  int i, j;
  BUCKETS *Buckets;
...
#define MINALPHA (1e-200)
{
  ...
  OldChiSquared = (CHISTRUCT *)first_node(
  ...
  if (OldChiSquared == NULL) {
    ...
    OldChiSquared->ChiSquared =
        Solve(ChiArea, OldChiSquared, (FLOAT64)DegreesOfFreedom,
              (FLOAT64)CHIACCURACY);
...
BOOL8 DistributionOK(BUCKETS *Buckets) {
  ...
  for (i = 0; i < Buckets->NumberOfBuckets; i++) {
    ...
    TotalDifference +=
        (FrequencyDifference * FrequencyDifference) / Buckets->ExpectedCount[i];
  }
...
void AdjustBuckets(BUCKETS *Buckets, uinT32 NewSampleCount) {
  ...
  AdjustFactor = (((FLOAT64)NewSampleCount) / ((FLOAT64)Buckets->SampleCount));
 ...
 */
FLOAT64
Solve(SOLVEFUNC Function, void *FunctionParams, FLOAT64 InitialGuess,
      FLOAT64 Accuracy)
...
#define DELTARATIO 0.1
{
  ...
  while (Abs(LastPosX - LastNegX) > Accuracy) {
    ...
    Slope = ((*Function)((CHISTRUCT *)FunctionParams, x + Delta) - f) / Delta;
 ...
 */
BOOL8
MultipleCharSamples(CLUSTERER *Clusterer, CLUSTER *Cluster, FLOAT32 MaxIllegal)
