...
EXTERN BOOL_VAR(oldbl_xhfix, FALSE, "Fix bug in modes threshold for xheights");
...
EXTERN INT_VAR(oldbl_holed_losscount, 10, "Max lost before fallback line used");
...
void Textord::find_textlines(TO_BLOCK *block,   // block row is in
  ...
  if (holed_line)
    ...
    make_first_baseline(blobcoords, blobcount, xcoords, ycoords, spline,
                        &row->baseline, jumplimit);
#ifndef GRAPHICS_DISABLED
  if (textord_show_final_rows)
    row->baseline.plot(to_win, ScrollView::GOLDENROD);
#endif
  if (blobcount > 1) {
    bestpart = partition_line(blobcoords, blobcount, &partcount, partids,
                              partsizes, &row->baseline, jumplimit, ydiffs);
    pointcount = partition_coords(blobcoords, blobcount, partids, bestpart,
                                  xcoords, ycoords);
    segments = segment_spline(blobcoords, blobcount, xcoords, ycoords, degree,
                              pointcount, xstarts);
    if (!holed_line) {
      do {
        row->baseline =
            QSPLINE(xstarts, segments, xcoords, ycoords, pointcount, degree);
      } while (textord_oldbl_split_splines &&
               split_stepped_spline(&row->baseline, jumplimit / 2, xcoords,
                                    xstarts, segments));
    }
    find_lesser_parts(row, blobcoords, blobcount, partids, partsizes, partcount,
                      bestpart);
  ...
  row->baseline.extrapolate(row->line_m(), block->block->bounding_box().left(),
                            block->block->bounding_box().right());

  if (textord_really_old_xheight) {
    old_first_xheight(row, blobcoords, lineheight, blobcount, &row->baseline,
                      jumplimit);
    ...
    compute_row_xheight(row, block->block->classify_rotation(), row->line_m(),
                        block->line_size);
  }
  free_mem(partids);
  free_mem(xcoords);
...
void make_first_baseline( // initial approximation
  ...
  if (spline == NULL          /*no given spline */
      ...
      spline->xcoords[spline->segments - 1] <
          rightedge - MAXOVERLAP * (rightedge - leftedge)) {
    if (textord_oldbl_paradef)
      return; // use default
    xstarts[0] = blobcoords[0].left() - 1;
    for (blobindex = 0; blobindex < blobcount; blobindex++) {
      xcoords[blobindex] =
          (blobcoords[blobindex].left() + blobcoords[blobindex].right()) / 2;
        ...
        for (blobindex = 0, segment = 1; blobindex < ycount; blobindex++) {
          if (yturns[blobindex] > minmin + jumplimit ||
            ...
            /*bigger max */
                     ...
                     /*smaller min */
                     || (prevy < maxmax - jumplimit &&
                         yturns[blobindex] < prevy)) {
    ...
    *baseline = *spline; /*copy it */
    shift = ICOORD(
        0, (inT16)(blobcoords[0].bottom() - spline->y(blobcoords[0].right())));
    baseline->move(shift);
  }
}
...
int partition_line(    // partition blobs
  ...
  *numparts = 1; /*1 partition */
  ...
  for (blobindex = startx; blobindex < blobcount; blobindex++) {
    ...
    bestpart = choose_partition(diff, partdiffs, bestpart, jumplimit, &drift,
                                &last_delta, numparts);
    /*record partition */
  ...
  if (textord_oldbl_merge_parts)
    merge_oldbl_parts(blobcoords, blobcount, partids, partsizes, biggestpart,
                      jumplimit);
    ...
    if (partids[blobindex] != prevpart) {
      ...
      if (prevpart != biggestpart && runlength > MAXBADRUN) {
        ...
        for (test_blob = startx; test_blob < blobindex; test_blob++) {
          coord = FCOORD(
              (blobcoords[test_blob].left() + blobcoords[test_blob].right()) /
                  2.0,
        ...
        found_one = FALSE;
        close_one = FALSE;
        for (test_blob = 1; !found_one && (startx - test_blob >= 0 ||
                                           blobindex + test_blob <= blobcount);
             test_blob++) {
          if (startx - test_blob >= 0 &&
              partids[startx - test_blob] == biggestpart) {
            found_one = TRUE;
            coord = FCOORD((blobcoords[startx - test_blob].left() +
                            blobcoords[startx - test_blob].right()) /
                               2.0,
        ...
        if (close_one) {
          if (textord_oldbl_debug)
            tprintf("Merged %d blobs back into part %d from %d starting at "
                    "(%d,%d)\n",
...
int get_ydiffs(        // evaluate differences
  ...
  for (blobindex = 0; blobindex < blobcount; blobindex++) {
    /*centre of blob */
    xcentre =
        (blobcoords[blobindex].left() + blobcoords[blobindex].right()) >> 1;
...
int segment_spline(             // make xstarts
  ...
  if (pointcount > 3) {
    ...
    while (ptindex < pointcount - 1 && turncount < SPLINESIZE - 1) {
      /*minimum */
      if (ycoords[ptindex - 1] > ycoords[ptindex] &&
          ycoords[ptindex] <= ycoords[ptindex + 1]) {
  ...
  if (textord_oldbl_debug && turncount > 0)
    tprintf("First turn is %d at (%d,%d)\n", turnpoints[0],
            xcoords[turnpoints[0]], ycoords[turnpoints[0]]);
  for (segment = 1; segment < turncount; segment++) {
    /*centre y coord */
    lastmax =
        (ycoords[turnpoints[segment - 1]] + ycoords[turnpoints[segment]]) / 2;
    ...
    if (ycoords[turnpoints[segment - 1]] < ycoords[turnpoints[segment]])
      ...
      for (ptindex = turnpoints[segment - 1] + 1;
           ptindex < turnpoints[segment] && ycoords[ptindex + 1] <= lastmax;
           ptindex++)
        ;
    else
    ...
    /*centre x */
    xstarts[segment] =
        (xcoords[ptindex - 1] + xcoords[ptindex] +
         xcoords[turnpoints[segment - 1]] + xcoords[turnpoints[segment]] + 2) /
        4;
    /*halfway between turns */
    if (textord_oldbl_debug)
      tprintf("Turn %d is %d at (%d,%d), mid pt is %d@%d, final @%d\n", segment,
              turnpoints[segment], xcoords[turnpoints[segment]],
              ycoords[turnpoints[segment]], ptindex - 1, xcoords[ptindex - 1],
              xstarts[segment]);
  }
  ...
  for (segment = 1; segment < segments - 1; segment++) {
    ...
    if (step > jumplimit) {
      ...
      if (segments >= SPLINESIZE) {
        ...
        insert_spline_point(
            xstarts, segment, (xcoords[leftindex - 1] + xcoords[leftindex]) / 2,
...
void find_lesser_parts( // get descenders
  ...
  for (runlength = 0, blobindex = 0; blobindex < blobcount; blobindex++) {
    ...
    if (part_id != bestpart) {
      ...
      partsteps[part_id] +=
          blobcoords[blobindex].bottom() - row->baseline.y(xcentre);
    } else
      runlength = 0;
  }
...
void old_first_xheight( // the wiseowl way
  ...
  if (blobcount > 1) {
    ...
    lineheight =
        (int)(blobcoords[0].top() -
              baseline->y((blobcoords[0].left() + blobcoords[0].right()) / 2) +
              0.5);
  }
...
void make_first_xheight( // find xheight
  ...
  for (blobindex = 0; blobindex < blobcount; blobindex++) {
    ...
    int strength = textord_ocropus_mode && bottomdiff <= kBaselineTouch
                       ? kGoodStrength
                       : 1;
...
void find_top_modes(            // get modes
  ...
  int total_max = 0;
  int mode_factor =
      textord_ocropus_mode ? kMinModeFactorOcropus : kMinModeFactor;
