...
ColPartition::ColPartition(BlobRegionType blob_type, const ICOORD &vertical)
    : left_margin_(-MAX_INT32), right_margin_(MAX_INT32),
      ...
      good_width_(false), good_column_(false), left_key_tab_(false),
      right_key_tab_(false), left_key_(0), right_key_(0), type_(PT_UNKNOWN),
      vertical_(vertical), working_set_(NULL), last_add_was_vertical_(false),
      block_owned_(false), desperately_merged_(false), first_column_(-1),
      last_column_(-1), column_set_(NULL), side_step_(0), top_spacing_(0),
      bottom_spacing_(0), type_before_table_(PT_UNKNOWN),
      inside_table_column_(false), nearest_neighbor_above_(NULL),
      nearest_neighbor_below_(NULL), space_above_(0), space_below_(0),
      space_to_left_(0), space_to_right_(0), owns_blobs_(true) {
...
void ColPartition::SetPartitionType(int resolution, ColPartitionSet *columns) {
  ...
  ColumnSpanningType span_type = columns->SpanningType(
      ...
      MIN(bounding_box_.height(), bounding_box_.width()), MidY(), left_margin_,
      right_margin_, &first_column_, &last_column_, &first_spanned_col);
  column_set_ = columns;
    ...
    if (it.empty() || part->bottom_spacing() > same_block_threshold ||
      ...
      if (!it.empty() && part->bottom_spacing() <= same_block_threshold) {
        ...
        if (part->SizesSimilar(*next_part) &&
          ...
          if (third_part == NULL || !next_part->SizesSimilar(*third_part) ||
...
void ColPartition::Print() const {
  ...
  tprintf("ColPart:%c(M%d-%c%d-B%d/%d,%d/%d)->(%dB-%d%c-%dM/%d,%d/%d)"
          ...
          LeftAtY(y), bounding_box_.left(), median_left_,
          bounding_box_.bottom(), median_bottom_, bounding_box_.right(),
          RightAtY(y), right_key_tab_ ? 'T' : 'B', right_margin_, median_right_,
...
bool ColPartition::ThisPartitionBetter(BLOBNBOX *bbox,
  ...
  int other_overlap =
      MIN(top, other.median_top_) - MAX(bottom, other.median_bottom_);
...
bool ColPartition::SpacingsEqual(const ColPartition &other,
  ...
  int top_error =
      MAX(TopSpacingMargin(resolution), other.TopSpacingMargin(resolution));
