...
template <typename T> void DeleteObject(T *object) { delete object; }

TableFinder::TableFinder()
    : resolution_(0), global_median_xheight_(0), global_median_blob_width_(0),
      global_median_ledding_(0), left_to_right_language_(true) {}
...
void TableFinder::FilterParagraphEndings() {
  ...
  while ((part = gsearch.NextFullSearch()) != NULL) {
    ...
    if (left_to_right_language_) {
      ...
      int left =
          MIN(part->bounding_box().left(), upper_part->bounding_box().left());
      ...
      int right =
          MAX(part->bounding_box().right(), upper_part->bounding_box().right());
...
ColSegment::ColSegment()
    : ELIST_LINK(), num_table_cells_(0), num_text_cells_(0),
      type_(COL_UNKNOWN) {}
ColSegment::~ColSegment() {}
