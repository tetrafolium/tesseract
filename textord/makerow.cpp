...
static float MakeRowFromBlobs(float line_size, BLOBNBOX_IT *blob_it,
                              TO_ROW_IT *row_it) {
...
#endif
  // guess skew
  assign_blobs_to_rows(block, NULL, 0, TRUE, TRUE,
                       textord_show_initial_rows && testing_on);
...
void compute_page_skew(    // get average gradient
  ...
  row_count = row_index;
  row_index = choose_nth_item((inT32)(row_count * textord_skew_ile), gradients,
                              row_count);
  page_m = gradients[row_index];
  row_index =
      choose_nth_item((inT32)(row_count * textord_skew_ile), errors, row_count);
...
static bool dot_of_i(BLOBNBOX *dot, BLOBNBOX *i, TO_ROW *row) {
  ...
  int overlap =
      MIN(dotbox.right(), ibox.right()) - MAX(dotbox.left(), ibox.left());
...
#ifndef GRAPHICS_DISABLED
      if (testing_on)
        plot_parallel_row(row, gradient, block_edge, ScrollView::WHITE,
                          rotation);
#endif
      blob_it.add_list_after(row_it.data()->blob_list());
...
BOOL8 find_best_dropout_row( // find neighbours
  ...
  if (testing_on)
    tprintf("Row at %g(%g), dropout dist=%d,", row->intercept(),
            row->parallel_c(), distance);
  if (distance < 0) {
    row_inc = 1;
    abs_dist = -distance;
  } else {
        ...
        if (row->believability() <= next_row->believability()) {
          if (testing_on) {
            tprintf(" equal but more believable at %g (%g/%g)\n",
                    next_row->intercept(), row->believability(),
...
#ifndef GRAPHICS_DISABLED
            if (textord_show_expanded_rows && testing_on)
              plot_parallel_row(test_row, gradient, block_edge,
...
void compute_row_stats( // find lines
  ...
  if (testing_on)
    tprintf("Blob based spacing=(%g,%g), offset=%g", block->line_size,
  ...
  if (rowcount > 0) {
    ...
    row_index = choose_nth_item(rowcount / 4, rows, rowcount, sizeof(TO_ROW *),
                                row_spacing_order);
    iqr -= rows[row_index]->spacing;
    row_index = choose_nth_item(rowcount / 2, rows, rowcount, sizeof(TO_ROW *),
                                row_spacing_order);
    ...
    block->baseline_offset =
        fmod(rows[row_index]->intercept(), block->line_spacing);
  }
  if (testing_on)
...
void Textord::compute_block_xheight(TO_BLOCK *block, float gradient) {
  TO_ROW *row; // current row
  float asc_frac_xheight =
      CCStruct::kAscenderFraction / CCStruct::kXHeightFraction;
  float desc_frac_xheight =
      CCStruct::kDescenderFraction / CCStruct::kXHeightFraction;
  ...
  for (row_it.mark_cycle_pt(); !row_it.cycled_list(); row_it.forward()) {
    ...
    if (row->xheight <= 0.0) {
      compute_row_xheight(row, block->block->classify_rotation(), gradient,
                          block->line_size);
    }
...
void Textord::compute_row_xheight(TO_ROW *row, // row to do
  ...
  fill_heights(row, gradient, min_height, max_height, &heights,
               &floating_heights);
  row->ascrise = 0.0f;
  row->xheight = 0.0f;
  row->xheight_evidence = compute_xheight_from_modes(
      &heights, &floating_heights,
      textord_single_height_mode && rotation.y() == 0.0, min_height, max_height,
 ...
 */
void fill_heights(TO_ROW *row, float gradient, int min_height, int max_height,
  ...
  do {
    ...
    if (!blob->joined_to_prev()) {
      xcentre =
          (blob->bounding_box().left() + blob->bounding_box().right()) / 2.0f;
 ...
 */
int compute_xheight_from_modes(STATS *heights, STATS *floating_heights,
                               bool cap_only, int min_height, int max_height,
                               float *xheight, float *ascrise) {
  ...
  int best_count = 0;
  int mode_count = compute_height_modes(heights, min_height, max_height, modes,
                                        MAX_HEIGHT_MODES);
  ...
  for (x = 0; x < mode_count - 1; x++) {
    ...
    int modes_x_count =
        heights->pile_count(modes[x]) - floating_heights->pile_count(modes[x]);
        ...
        if (textord_ascx_ratio_min < ratio && ratio < textord_ascx_ratio_max &&
          ...
          if (textord_debug_xheights) {
            tprintf("X=%d, asc=%d, count=%d, ratio=%g\n", modes[x],
                    modes[asc] - modes[x], modes_x_count, ratio);
          }
          prev_size = modes[x];
 ...
 */
inT32 compute_row_descdrop(TO_ROW *row, float gradient, int xheight_blob_count,
                           STATS *asc_heights) {
  ...
  if ((i_min / row->xheight) < textord_ascx_ratio_min) {
    i_min =
        static_cast<int>(floor(row->xheight * textord_ascx_ratio_min + 0.5));
  }
 ...
 */
void correct_row_xheight(TO_ROW *row, float xheight, float ascrise,
                         float descdrop) {
  ROW_CATEGORY row_category = get_row_category(row);
  if (textord_debug_xheights) {
  ...
  bool cap_xheight = within_error_margin(row->xheight, xheight + ascrise,
...
void separate_underlines(TO_BLOCK *block,    // block to do
  ...
  TO_ROW_IT row_it = block->get_rows();
  int min_blob_height = static_cast<int>(
      textord_min_blob_height_fraction * block->line_size + 0.5);
  ...
  for (row_it.mark_cycle_pt(); !row_it.cycled_list(); row_it.forward()) {
    ...
    for (blob_it.mark_cycle_pt(); !blob_it.cycled_list(); blob_it.forward()) {
      ...
      if (blob_box.width() > block->line_size * textord_underline_width) {
        ...
        if (test_underline(
                testing_on && textord_show_final_rows, rotated_blob,
...
double *linear_spline_baseline( // split baseline
  ...
  if (textord_oldbl_debug)
    tprintf("Linear splining baseline of %d blobs at (%d,%d), into %d segments "
            "of %d blobs\n",
...
void assign_blobs_to_rows( // find lines
  ...
  ycoord = (block->block->bounding_box().bottom() +
            block->block->bounding_box().top()) /
           2.0f;
  if (gradient != NULL)
  ...
  for (blob_it.mark_cycle_pt(); !blob_it.cycled_list(); blob_it.forward()) {
    ...
    if (gradient != NULL) {
      ...
      block_skew *=
          (float)(blob->bounding_box().left() - left_x) / (last_x - left_x);
      //                      tprintf("to %g\n",block_skew);
    }
    ...
    if (!row_it.empty()) {
      for (row_it.move_to_first();
           !row_it.at_last() && row_it.data()->min_y() > top; row_it.forward())
        ;
      row = row_it.data();
      if (row->min_y() <= top && row->max_y() >= bottom) {
        // any overlap
        dest_row = row;
        overlap_result = most_overlapping_row(
            &row_it, dest_row, top, bottom, block->line_size,
      ...
      if (overlap_result == NEW_ROW) {
        if (make_new_rows && top - bottom < block->max_blob_size) {
          ...
          smooth_factor =
              1.0 / (row_count * textord_skew_lag + textord_skewsmooth_offset);
        } else
      ...
      row_it.add_after_then_move(dest_row);
      smooth_factor =
          1.0 / (row_count * textord_skew_lag + textord_skewsmooth_offset2);
    } else
      overlap_result = REJECT;
...
OVERLAP_STATE most_overlapping_row( // find best row
  ...
  if (testing_blob && textord_debug_blob) {
    tprintf("Test blob y=(%g,%g), row=(%f,%f), size=%g, overlap=%f\n", bottom,
            top, row->min_y(), row->max_y(), rowsize, bestover);
  }
  test_row = row;
  do {
    if (!row_it->at_last()) {
      ...
      if (test_row->min_y() <= top && test_row->max_y() >= bottom) {
        merge_top =
            test_row->max_y() > row->max_y() ? test_row->max_y() : row->max_y();
        merge_bottom =
            test_row->min_y() < row->min_y() ? test_row->min_y() : row->min_y();
        if (merge_top - merge_bottom <= rowsize) {
          if (testing_blob) {
            tprintf("Merging rows at (%g,%g), (%g,%g)\n", row->min_y(),
                    row->max_y(), test_row->min_y(), test_row->max_y());
          }
        ...
        if (testing_blob && textord_debug_blob) {
          tprintf("Test blob y=(%g,%g), row=(%f,%f), size=%g, overlap=%f->%f\n",
                  bottom, top, test_row->min_y(), test_row->max_y(), rowsize,
                  overlap, bestover);
        }
      }
    }
  } while (!row_it->at_last() && test_row->min_y() <= top &&
           test_row->max_y() >= bottom);
...
void mark_repeated_chars(TO_ROW *row) {
  ...
  if (!box_it.empty()) {
    do {
      ...
      int repeat_length = 1;
      if (bblob->flow() == BTFT_LEADER && !bblob->joined_to_prev() &&
          bblob->cblob() != NULL) {
