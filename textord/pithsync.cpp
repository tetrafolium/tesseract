      ...
      if (!segpt->terminal && segpt->fake_count < MAX_INT16) {
        ...
        if (textord_balance_factor > 0) {
          if (textord_fast_pitch_test) {
            ...
            for (balance_index = 0; index + balance_index < x - balance_index;
                 balance_index++)
              balance_count +=
  ...
  if (index >= array_origin) {
    ...
    if (!segpt->terminal && segpt->fake_count < MAX_INT16) {
      ...
      if (textord_balance_factor > 0) {
        ...
        balance_count =
            (inT16)(balance_count * textord_balance_factor / projection_scale);
      }
      r_index = segpt->region_index + 1;
      total = segpt->mean_sum + dist;
      balance_count += offset;
...
double check_pitch_sync2(  // find segmentation
    ...
    inT16 projection_right, float projection_scale,
  ...
  for (left_edge = projection_left;
       projection->pile_count(left_edge) == 0 && left_edge < projection_right;
       left_edge++)
    ;
  for (right_edge = projection_right;
       projection->pile_count(right_edge) == 0 && right_edge > left_edge;
       right_edge--)
    ;
  ...
  if (pitsync_linear_version >= 4)
    return check_pitch_sync3(projection_left, projection_right, zero_count,
                             pitch, pitch_error, projection, projection_scale,
                             occupation_count, seg_list, start, end);
  array_origin = left_edge - pitch;
  cutpts = (FPCUTPT *)alloc_mem((right_edge - left_edge + pitch * 2 + 1) *
                                sizeof(FPCUTPT));
  for (x = array_origin; x < left_edge; x++)
    // free cuts
    cutpts[x - array_origin].setup(cutpts, array_origin, projection, zero_count,
                                   pitch, x, 0);
  ...
  while (x < right_edge - pitch_error) {
    ...
    cutpts[x - array_origin].assign(cutpts, array_origin, x, faking, mid_cut,
                                    offset, projection, projection_scale,
                                    zero_count, pitch, pitch_error);
    x++;
  }
  ...
  while (x < right_edge + pitch) {
    ...
    cutpts[x - array_origin].assign(cutpts, array_origin, x, FALSE, FALSE,
                                    offset, projection, projection_scale,
                                    zero_count, pitch, pitch_error);
    cutpts[x - array_origin].terminal = TRUE;
    if (cutpts[x - array_origin].index() +
            cutpts[x - array_origin].fake_count <=
  ...
  if (this_box.right() == textord_test_x && this_box.top() == textord_test_y) {
    for (x = left_edge - pitch; x < right_edge + pitch; x++) {
      tprintf("x=%d, C=%g, s=%g, sq=%g, prev=%d\n", x,
              cutpts[x - array_origin].cost_function(),
...
double check_pitch_sync3(  // find segmentation
    ...
    inT16 projection_right, inT16 zero_count,
    inT16 pitch,             // pitch estimate
    inT16 pitch_error,       // tolerance
    STATS *projection,       // vertical
  ...
  while (x < right_edge - pitch_error) {
    ...
    if ((start == 0 && end == 0) || !textord_fast_pitch_test ||
      ...
      cutpts[x - array_origin].assign_cheap(
          cutpts, array_origin, x, faking, mid_cut, offset, projection,
