...
BaselineRow::BaselineRow(double line_spacing, TO_ROW *to_row)
    : blobs_(to_row->blob_list()), baseline_pt1_(0.0f, 0.0f),
      baseline_pt2_(0.0f, 0.0f), baseline_error_(0.0), good_baseline_(false) {
  ComputeBoundingBox();
...
double BaselineRow::SpaceBetween(const BaselineRow &other) const {
  ...
  float x = (MAX(bounding_box_.left(), other.bounding_box_.left()) +
             MIN(bounding_box_.right(), other.bounding_box_.right())) /
            2.0f;
...
double
BaselineBlock::FitLineSpacingModel(const GenericVector<double> &positions,
