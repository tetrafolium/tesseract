...
ScrollView *create_to_win(ICOORD page_tr) {
  if (to_win != NULL)
    return to_win;
  to_win =
      new ScrollView(TO_WIN_NAME, TO_WIN_XPOS, TO_WIN_YPOS, page_tr.x() + 1,
                     page_tr.y() + 1, page_tr.x(), page_tr.y(), true);
  return to_win;
}
...
void draw_occupation(                // draw projection
  ...
  for (line_index = min_y; line_index <= max_y; line_index++) {
    ...
    to_win->DrawTo(fleft + occupation[line_index - min_y] / 10.0,
                   (float)line_index);
  }
  ...
  to_win->SetCursor(fleft, (float)ybottom);
  for (line_index = min_y; line_index <= max_y; line_index++) {
    to_win->DrawTo(fleft + thresholds[line_index - min_y] / 10.0,
                   (float)line_index);
  }
}
...
void draw_meanlines(          // draw a block
  ...
  for (row_it.mark_cycle_pt(); !row_it.cycled_list(); row_it.forward()) {
    ...
    plot_pt =
        FCOORD((float)left, gradient * left + row->parallel_c() + row->xheight);
    plot_pt.rotate(rotation);
    to_win->SetCursor(plot_pt.x(), plot_pt.y());
    plot_pt = FCOORD((float)right,
  ...
  for (blob_it.mark_cycle_pt(); !blob_it.cycled_list(); blob_it.forward()) {
    ...
    if (!blob->joined_to_prev() &&
      ...
      if (blob_box.left() - prev_x < row->min_space) {
        ...
        win->Rectangle(prev_x, blob_box.bottom(), blob_box.left(),
                       blob_box.top());
      }
    }
    if (!blob->joined_to_prev())
...
void plot_fp_cells(           // draw words
  ...
  if (pitsync_linear_version)
    check_pitch_sync2(blob_it, blob_count, pitch, 2, projection,
                      projection_left, projection_right, projection_scale,
  ...
  for (seg_it.mark_cycle_pt(); !seg_it.cycled_list(); seg_it.forward()) {
    ...
    win->Line(segpt->position(), word_box.bottom(), segpt->position(),
              word_box.top());
  }
}
...
void plot_row_cells(          // draw words
  ...
  for (cell_it.mark_cycle_pt(); !cell_it.cycled_list(); cell_it.forward()) {
    cell = cell_it.data();
    win->Line(cell->x() + xshift, word_box.bottom(), cell->x() + xshift,
              word_box.top());
  }
}
