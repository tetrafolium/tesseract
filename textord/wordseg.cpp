...
void set_row_spaces( // find space sizes
  ...
  for (row_it.mark_cycle_pt(); !row_it.cycled_list(); row_it.forward()) {
    ...
    if (row->fixed_pitch == 0) {
      row->min_space =
          (inT32)ceil(row->pr_space - (row->pr_space - row->pr_nonsp) *
                                          textord_words_definite_spread);
      row->max_nonspace =
          (inT32)floor(row->pr_nonsp + (row->pr_space - row->pr_nonsp) *
                                           textord_words_definite_spread);
...
inT32 row_words(     // compute space size
  ...
  if (cluster_count > 2) {
    if (testing_on && textord_show_initial_words) {
      tprintf("Row at %g has 3 sizes of gap:%g,%g,%g\n", row->intercept(),
              cluster_stats[1].ile(0.5), cluster_stats[2].ile(0.5),
              cluster_stats[3].ile(0.5));
    }
    lower = gaps[0];
    if (gaps[1] > lower) {
  ...
  if (testing_on && textord_show_initial_words) {
    ...
    tprintf("Row at %g has minspace=%d(%g), max_non=%d(%g)\n", row->intercept(),
            row->min_space, upper, row->max_nonspace, lower);
  }
...
inT32 row_words2(    // compute space size
  ...
  for (gap_index = 0;
       gap_index < cluster_count && gaps[gap_index] > block->max_nonspace;
       gap_index++)
    ;
...
void make_real_words(tesseract::Textord *textord,
  ...
  block->block->set_stats(block->fixed_pitch == 0, (inT16)block->kern_size,
                          (inT16)block->space_size, (inT16)block->fixed_pitch);
