  ...
  if (bad_row->pitch_decision != PITCH_DEF_FIXED &&
    ...
    if (block_votes > textord_words_veto_power) {
      ...
      if (block_votes == 0 && like_votes == 0 && other_votes > 0 &&
        ...
        tprintf("Warning:row %d of block %d set prop with no like rows against "
                "trend\n",
                row_target, block_target);
    }
  }
  if (textord_debug_pitch_metric) {
    tprintf(":b_votes=%d:l_votes=%d:o_votes=%d", block_votes, like_votes,
            other_votes);
    tprintf("x=%g:asc=%g\n", bad_row->xheight, bad_row->ascrise);
  }
  if (bad_row->pitch_decision == PITCH_CORR_FIXED) {
    ...
    if (bad_row->char_cells.empty() && !bad_row->blob_list()->empty()) {
      tune_row_pitch(bad_row, &bad_row->projection, bad_row->projection_left,
                     ...
                     (bad_row->fixed_pitch + bad_row->max_nonspace * 3) / 4,
                     bad_row->fixed_pitch, sp_sd, mid_cuts,
                     &bad_row->char_cells, FALSE);
    }
...
void compute_block_pitch(TO_BLOCK *block,    // input list
  ...
  if (testing_on && textord_debug_pitch_test) {
    tprintf("Block %d at (%d,%d)->(%d,%d)\n", block_index, block_box.left(),
            block_box.bottom(), block_box.right(), block_box.top());
  }
  block->min_space =
      (inT32)floor(block->xheight * textord_words_default_minspace);
  block->max_nonspace =
      (inT32)ceil(block->xheight * textord_words_default_nonspace);
...
BOOL8 compute_rows_pitch( // find line stats
  ...
  for (row_it.mark_cycle_pt(); !row_it.cycled_list(); row_it.forward()) {
    ...
    if (row_pitch_stats(row, maxwidth, testing_on) &&
        find_row_pitch(row, maxwidth, textord_dotmatrix_gap + 1, block,
                       block_index, row_index, testing_on)) {
...
BOOL8 try_doc_fixed(            // determine pitch
  ...
  for (block_it.mark_cycle_pt(); !block_it.cycled_list(); block_it.forward()) {
    ...
    for (row_it.mark_cycle_pt(); !row_it.cycled_list(); row_it.forward()) {
      ...
      row_left =
          (inT16)(row->projection_left - shift_factor * (master_y - row_y));
      row_right =
          (inT16)(row->projection_right - shift_factor * (master_y - row_y));
  ...
  if (textord_show_page_cuts && to_win != NULL)
    projection.plot(to_win, projection_left, row->intercept(), 1.0f, -1.0f,
                    ScrollView::CORAL);
#endif
  final_pitch = pitches.ile(0.5);
  pitch = (inT16)final_pitch;
  pitch_sd = tune_row_pitch(row, &projection, projection_left, projection_right,
                            pitch * 0.75, final_pitch, sp_sd, mid_cuts,
                            &row->char_cells, FALSE);

  if (textord_debug_pitch_metric)
    tprintf("try_doc:props=%d:fixed=%d:pitch=%d:final_pitch=%g:pitch_sd=%g:sp_"
            "sd=%g:sd/trc=%g:sd/p=%g:sd/trc/p=%g\n",
  ...
  if (textord_show_page_cuts && to_win != NULL) {
    ...
    for (block_it.mark_cycle_pt(); !block_it.cycled_list();
      ...
      for (row_it.mark_cycle_pt(); !row_it.cycled_list(); row_it.forward()) {
        ...
        plot_row_cells(to_win, ScrollView::GOLDENROD, row, row_shift,
                       master_cells);
      }
    }
  }
...
BOOL8 try_rows_fixed(  // find line stats
  ...
  count_block_votes(block, def_fixed, def_prop, maybe_fixed, maybe_prop,
                    corr_fixed, corr_prop, dunno);
  if (testing_on && (textord_debug_pitch_test || textord_blocksall_prop ||
...
void print_block_counts( // find line stats
  ...
  count_block_votes(block, def_fixed, def_prop, maybe_fixed, maybe_prop,
                    corr_fixed, corr_prop, dunno);
  tprintf("Block %d has (%d,%d,%d)", block_index, def_fixed, maybe_fixed,
          corr_fixed);
...
void count_block_votes( // find line stats
    ...
    inT32 &def_prop, inT32 &maybe_fixed, inT32 &maybe_prop, inT32 &corr_fixed,
    inT32 &corr_prop, inT32 &dunno) {
  TO_ROW *row; // current row
  TO_ROW_IT row_it = block->get_rows();
...
BOOL8 row_pitch_stats( // find line stats
  ...
  for (gap_index = 0; gap_index < cluster_count && gaps[gap_index] < lower;
       gap_index++)
    ;
  if (gap_index == 0) {
    if (testing_on)
  ...
  for (gap_index = 0; gap_index < cluster_count && gaps[gap_index] < upper;
       gap_index++)
    ;
  if (gap_index == 0) {
    if (testing_on)
  ...
  if (testing_on) {
    tprintf("Initial estimates:pr_nonsp=%g, pr_space=%g, fp_nonsp=%g, "
            "fp_space=%g\n",
...
BOOL8 find_row_pitch(  // find lines
  ...
  if (!count_pitch_stats(row, &gap_stats, &pitch_stats, initial_pitch,
                         min_space, TRUE, FALSE, dm_gap)) {
    dm_gap_iqr = 0.0001;
  ...
  if (!count_pitch_stats(row, &gap_stats, &pitch_stats, initial_pitch,
    ...
    if (testing_on)
      tprintf("First fp iteration:initial_pitch=%g, gap_iqr=%g, pitch_iqr=%g, "
              "pitch=%g\n",
    ...
    if (min_space > initial_pitch &&
      ...
      pitch_iqr = pitch_stats.ile(0.75) - pitch_stats.ile(0.25);
      if (testing_on)
        tprintf("Revised fp iteration:initial_pitch=%g, gap_iqr=%g, "
                "pitch_iqr=%g, pitch=%g\n",
                initial_pitch, gap_iqr, pitch_iqr, pitch_stats.ile(0.5));
      initial_pitch = pitch_stats.ile(0.5);
    }
  ...
  if (textord_debug_pitch_metric)
    tprintf(
        ...
        block_index, row_index, 'X', pitch_iqr, gap_iqr, dm_pitch_iqr,
        dm_gap_iqr,
  ...
  if (pitch_iqr * dm_gap_iqr <= dm_pitch_iqr * gap_iqr) {
    if (testing_on)
      tprintf("Choosing non dm version:pitch_iqr=%g, gap_iqr=%g, "
              "dm_pitch_iqr=%g, dm_gap_iqr=%g\n",
    ...
    if (testing_on)
      tprintf("Choosing dm version:pitch_iqr=%g, gap_iqr=%g, dm_pitch_iqr=%g, "
              "dm_gap_iqr=%g\n",
              pitch_iqr, gap_iqr, dm_pitch_iqr, dm_gap_iqr);
    gap_iqr = dm_gap_iqr;
  ...
  if (textord_debug_pitch_metric) {
    ...
    tprintf("p_iqr/g=%g:p_iqr/x=%g:iqr_res=%c:", pitch_iqr / gap_iqr,
            pitch_iqr / block->xheight,
...
BOOL8 fixed_pitch_row(TO_ROW *row, // row to do
  ...
  if (textord_all_prop || (pb != NULL && !pb->IsText())) {
    ...
    pitch_sd = tune_row_pitch(
        row, &row->projection, row->projection_left, row->projection_right,
    ...
    if (pitch_sd < textord_words_pitchsd_threshold * row->fixed_pitch &&
        ((pitsync_linear_version & 3) < 3 ||
         ((pitsync_linear_version & 3) >= 3 &&
          (row->used_dm_model || sp_sd > 20 ||
           (pitch_sd == 0 && sp_sd > 10))))) {
      if (pitch_sd < textord_words_def_fixed * row->fixed_pitch &&
          !row->all_caps && ((pitsync_linear_version & 3) < 3 || sp_sd > 20))
      ...
      if ((blob_box.left() - joined_box.right() < dm_gap &&
        ...
        if (split_outsize) {
          ...
          width_units = width < 1 + words_default_fixed_limit &&
                                width > 1 - words_default_fixed_limit
                            ? 0
                            : -1;
        } else
...
float tune_row_pitch(           // find fp cells
  ...
  if (textord_fast_pitch_test)
    return tune_row_pitch2(row, projection, projection_left, projection_right,
                           space_size, initial_pitch, best_sp_sd,
                           // space sd
  ...
  initial_sd = compute_pitch_sd(
      row, projection, projection_left, projection_right, space_size,
  ...
  if (textord_debug_pitch_metric)
    print_pitch_sd(row, projection, projection_left, projection_right,
                   space_size, best_pitch);
...
float tune_row_pitch2(          // find fp cells
  ...
  for (pitch_delta = -textord_pitch_range; pitch_delta <= textord_pitch_range;
       pitch_delta++)
    sum_proj[textord_pitch_range + pitch_delta].set_range(
        0, best_pitch + pitch_delta + 1);
  ...
  for (pitch_delta = -textord_pitch_range; pitch_delta <= textord_pitch_range;
    ...
    for (pixel = 0; pixel < best_pitch + pitch_delta; pixel++) {
      if (sum_proj[textord_pitch_range + pitch_delta].pile_count(pixel) <
          best_count) {
        best_count =
            sum_proj[textord_pitch_range + pitch_delta].pile_count(pixel);
  ...
  for (start = best_pixel - 2;
       ...
       sum_proj[textord_pitch_range + best_delta].pile_count(
           start % best_pitch) <= best_count;
       start--)
    ;
  for (end = best_pixel + 2;
       end < best_pixel + best_pitch &&
       sum_proj[textord_pitch_range + best_delta].pile_count(
           end % best_pitch) <= best_count;
       end++)
    ;

  best_sd = compute_pitch_sd(row, projection, projection_left, projection_right,
                             space_size, initial_pitch, best_sp_sd,
                             best_mid_cuts, best_cells, testing_on, start, end);
  if (testing_on)
    tprintf("tune_row_pitch:output pitch=%g, sd=%g\n", initial_pitch, best_sd);

  if (textord_debug_pitch_metric)
    print_pitch_sd(row, projection, projection_left, projection_right,
                   space_size, initial_pitch);
...
float compute_pitch_sd(        // find fp cells
  ...
  if ((pitsync_linear_version & 3) > 1) {
    word_sync = compute_pitch_sd2(row, projection, projection_left,
                                  projection_right, initial_pitch, occupation,
                                  mid_cuts, row_cells, testing_on, start, end);
    sp_sd = occupation;
    return word_sync;
  }
  mid_cuts = 0;
  ...
  do {
    ...
    if (pitsync_linear_version & 3)
      ...
      word_sync = check_pitch_sync(&start_it, blob_count, (inT16)initial_pitch,
                                   2, projection, &seg_list);
    if (testing_on) {
      tprintf("Word ending at (%d,%d), len=%d, sync rating=%g, ",
              prev_box.right(), prev_box.top(), seg_list.length() - 1,
              word_sync);
...
float compute_pitch_sd2(       // find fp cells
  ...
  plot_it = blob_it;
  word_sync = check_pitch_sync2(&blob_it, blob_count, (inT16)initial_pitch, 2,
                                projection, projection_left, projection_right,
                                row->xheight * textord_projection_scale,
                                occupation, &seg_list, start, end);
  if (testing_on) {
    tprintf("Row ending at (%d,%d), len=%d, sync rating=%g, ", blob_box.right(),
            blob_box.top(), seg_list.length() - 1, word_sync);
    seg_it.set_to_list(&seg_list);
    for (seg_it.mark_cycle_pt(); !seg_it.cycled_list(); seg_it.forward()) {
      if (seg_it.data()->faked)
        tprintf("(F)");
...
void print_pitch_sd(       // find fp cells
    ...
    inT16 projection_right, float space_size,
    float initial_pitch // guess at pitch
) {
  ...
  do {
    ...
    word_sync = check_pitch_sync2(
        &start_it, blob_count, (inT16)initial_pitch, 2, projection,
  ...
  tprintf("new_sd=%g:sd/p=%g:new_sp_sd=%g:res=%c:", word_sync,
          word_sync / initial_pitch, sp_sd,
          word_sync < textord_words_pitchsd_threshold * initial_pitch ? 'F'
                                                                      : 'P');

  start_it = row_start;
  blob_it = row_start;
  word_sync = check_pitch_sync2(
      &blob_it, total_blob_count, (inT16)initial_pitch, 2, projection,
      projection_left, projection_right,
      row->xheight * textord_projection_scale, occupation, &seg_list, 0, 0);
  if (occupation > 1)
  ...
  tprintf("row_sd=%g:sd/p=%g:res=%c:N=%d:res2=%s,init pitch=%g, row_pitch=%g, "
          "all_caps=%d\n",
