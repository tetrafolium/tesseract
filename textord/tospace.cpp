      ...
      if ((row->pitch_decision == PITCH_DEF_PROP) ||
        ...
        if ((tosp_debug_level > 0) && !old_text_ord_proportional)
          tprintf("Block %d Row %d: Now Proportional\n", block_index,
                  row_index);
        row_spacing_stats(row, gapmap, block_index, row_index,
                          block_space_gap_width, block_non_space_gap_width);
      } else {
        if ((tosp_debug_level > 0) && old_text_ord_proportional)
...
void Textord::block_spacing_stats(
    TO_BLOCK *block, GAPMAP *gapmap, BOOL8 &old_text_ord_proportional,
  ...
  for (row_it.mark_cycle_pt(); !row_it.cycled_list(); row_it.forward()) {
    ...
    if (!row->blob_list()->empty() &&
      ...
      while (!blob_it.cycled_list()) {
        ...
        if (!ignore_big_gap(row, row_length, gapmap, prev_blob_box.right(),
                            blob_box.left())) {
          all_gap_stats.add(gap_width, 1);

          centre_to_centre = (blob_box.left() + blob_box.right() -
                              (prev_blob_box.left() + prev_blob_box.right())) /
                             2;
          // DEBUG
  ...
  if (all_gap_stats.get_total() <= 1) {
    ...
    /* For debug only ..... */
    iqr_centre_to_centre =
        centre_to_centre_stats.ile(0.75) - centre_to_centre_stats.ile(0.25);
    ...
    */
      ...
      if (!row->blob_list()->empty() &&
        ...
        while (!blob_it.cycled_list()) {
          ...
          if ((gap_width > real_space_threshold) &&
              !ignore_big_gap(row, row_length, gapmap, prev_blob_box.right(),
                              blob_box.left())) {
            /*
    ...
    if (space_gap_stats.get_total() <= 2)
      ...
      block_space_gap_width = MAX((inT16)floor(space_gap_stats.median()),
...
void Textord::row_spacing_stats(
    TO_ROW *row, GAPMAP *gapmap, inT16 block_idx, inT16 row_idx,
  ...
  if ((cert_space_gap_stats.get_total() >=
    ...
    if (!tosp_recovery_isolated_row_stats ||
      ...
      if (tosp_row_use_cert_spaces1 && good_block_space_estimate) {
        ...
        old_to_method(row, &all_gap_stats, &all_space_gap_stats,
                      &small_gap_stats, block_space_gap_width,
                      block_non_space_gap_width);
    }
  }
  ...
  if (tosp_sanity_method == 0) {
    ...
    if ((row->space_size < tosp_min_sane_kn_sp * MAX(row->kern_size, 2.5)) ||
      ...
      if (good_block_space_estimate &&
        ...
        sane_space = MAX(tosp_min_sane_kn_sp * MAX(row->kern_size, 2.5),
                         row->xheight / 2);
      if (tosp_debug_level > 5)
      ...
      row->space_size = sane_space;
      row->space_threshold = inT32(
          floor((row->space_size + row->kern_size) / tosp_old_sp_kn_th_factor));
    }
    /* NEVER let threshold get VERY far away from kern */
    sane_threshold =
        inT32(floor(tosp_max_sane_kn_thresh * MAX(row->kern_size, 2.5)));
      ...
      if ((row->space_size < sane_space) ||
        ...
        if (tosp_debug_level > 5)
          tprintf("B:%d R:%d -- SUSPECT NO SPACES %3.2f %d %3.2f.\n", block_idx,
                  row_idx, row->kern_size, row->space_threshold,
                  row->space_size);
  ...
  if (tosp_old_to_method) {
    ...
    row->min_space = MIN(inT32(ceil(tosp_fuzzy_space_factor * row->xheight)),
  ...
  if ((tosp_table_fuzzy_kn_sp_ratio > 0) &&
    ...
    row->min_space =
        MAX(row->min_space,
            (inT32)ceil(tosp_table_fuzzy_kn_sp_ratio * row->kern_size));

  if ((tosp_fuzzy_kn_fraction > 0) && (row->kern_size < row->space_threshold)) {
    row->max_nonspace = (inT32)floor(
        0.5 + row->kern_size +
        tosp_fuzzy_kn_fraction * (row->space_threshold - row->kern_size));
  }
  ...
  if (tosp_debug_level > 5)
    tprintf("B:%d R:%d L:%d-- Kn:%d Sp:%d Thr:%d -- Kn:%3.2f (%d) Thr:%d (%d) "
            "Sp:%3.2f\n",
...
void Textord::old_to_method(
    TO_ROW *row, STATS *all_gap_stats, STATS *space_gap_stats,
  ...
  if (tosp_threshold_bias2 > 0) {
    row->space_threshold =
        inT32(floor(0.5 + row->kern_size +
                    tosp_threshold_bias2 * (row->space_size - row->kern_size)));
  } else {
    /*
  ...
  if (tosp_old_to_constrain_sp_kn && tosp_sanity_method == 1 &&
      ((row->space_size < tosp_min_sane_kn_sp * MAX(row->kern_size, 2.5)) ||
       ((row->space_size - row->kern_size) <
        tosp_silly_kn_sp_gap * row->xheight))) {
 ...
 * Set values for min_space, max_non_space based on row stats only
 *************************************************************************/
BOOL8 Textord::isolated_row_stats(TO_ROW *row, GAPMAP *gapmap,
                                  STATS *all_gap_stats, BOOL8 suspected_table,
                                  inT16 block_idx, inT16 row_idx) {
  float kern_estimate;
  ...
  crude_threshold_estimate = MAX(tosp_init_guess_kn_mult * kern_estimate,
                                 tosp_init_guess_xht_mult * row->xheight);
  small_gaps_count =
      stats_count_under(all_gap_stats, (inT16)ceil(crude_threshold_estimate));
  ...
  while (!blob_it.cycled_list()) {
    ...
    if (!ignore_big_gap(row, row_length, gapmap, prev_blob_box.right(),
      ...
      if ((gap_width > tosp_fuzzy_space_factor2 * row->xheight) ||
          ((gap_width > tosp_fuzzy_space_factor1 * row->xheight) &&
           (!tosp_narrow_blobs_not_cert || (!narrow_blob(row, prev_blob_box) &&
                                            !narrow_blob(row, blob_box)))) ||
          (wide_blob(row, prev_blob_box) && wide_blob(row, blob_box)))
        cert_space_gap_stats.add(gap_width, 1);
      all_space_gap_stats.add(gap_width, 1);
...
void Textord::improve_row_threshold(TO_ROW *row, STATS *all_gap_stats) {
  ...
  if ((all_gap_stats->get_total() <= 25) || (sp <= 10) || (sp <= 3 * kn) ||
      (stats_count_under(all_gap_stats, (inT16)ceil(kn + (sp - kn) / 3 + 0.5)) <
...
ROW *Textord::make_prop_words(TO_ROW *row,    // row to make
  ...
  if (!box_it.empty()) {
    ...
    if (xstarts[0] > next_rep_char_word_right) {
      ...
      current_gap =
          box_it.data()->bounding_box().left() - next_rep_char_word_right;
      ...
      if (tosp_debug_level > 5)
        tprintf("Repch wd at BOL(%d, %d). rep spacing %5.2f;  Rgap:%d  ",
                ...
                box_it.data()->bounding_box().bottom(), repetition_spacing,
                current_gap);
      prev_fuzzy_sp = FALSE;
      prev_fuzzy_non = FALSE;
      if (rep_char_it.empty()) {
    ...
    do {
      ...
      if (!bblob->joined_to_prev() && bblob->cblob() != NULL) {
        ...
        peek_at_next_gap(row, box_it, next_blob_box, next_gap,
                         next_within_xht_gap);
        ...
        if (blob_box.left() > next_rep_char_word_right ||
            make_a_word_break(
                row, blob_box, prev_gap_arg, prev_blob_box, current_gap,
                current_within_xht_gap, next_blob_box, next_gap_arg, blanks,
                fuzzy_sp, fuzzy_non, prev_gap_was_a_space, break_at_next_gap) ||
            box_it.at_first()) {
          ...
          if (blob_box.left() > next_rep_char_word_right) {
            ...
            if (tosp_debug_level > 5)
              tprintf("Repch wd (%d,%d) rep gap %5.2f;  Lgap:%d (%d blanks);",
                      ...
                      word->bounding_box().bottom(), repetition_spacing,
                      current_gap, blanks);
...
BOOL8 Textord::make_a_word_break(
    ...
    inT16 prev_gap, TBOX prev_blob_box, inT16 real_current_gap,
    inT16 within_xht_current_gap, TBOX next_blob_box, inT16 next_gap,
    uinT8 &blanks, BOOL8 &fuzzy_sp, BOOL8 &fuzzy_non,
    BOOL8 &prev_gap_was_a_space, BOOL8 &break_at_next_gap) {
  BOOL8 space;
  inT16 current_gap;
  ...
  if (tosp_old_to_method) {
    ...
    */
    if (tosp_use_xht_gaps && (real_current_gap <= row->max_nonspace) &&
      ...
      fuzzy_non = TRUE;
#ifndef GRAPHICS_DISABLED
      mark_gap(blob_box, 20, prev_gap, prev_blob_box.width(), current_gap,
               next_blob_box.width(), next_gap);
#endif
      ...
      if (tosp_flip_fuzz_kn_to_sp)
        ...
        fuzzy_non = TRUE;
#ifndef GRAPHICS_DISABLED
      mark_gap(blob_box, 21, prev_gap, prev_blob_box.width(), current_gap,
               next_blob_box.width(), next_gap);
#endif
      ...
      space = TRUE;
#ifndef GRAPHICS_DISABLED
      mark_gap(blob_box, 22, prev_gap, prev_blob_box.width(), current_gap,
               next_blob_box.width(), next_gap);
#endif
    ...
    /* Now continue with normal heuristics */
      ...
      if (tosp_pass_wide_fuzz_sp_to_context > 0)
        fuzzy_sp_to_kn_limit =
            row->kern_size + tosp_pass_wide_fuzz_sp_to_context *
                                 (row->space_size - row->kern_size);
      else
      ...
      if ((prev_blob_box.width() > 0) && narrow_blob(row, prev_blob_box) &&
        ...
        if ((tosp_all_flips_fuzzy) || (current_gap > fuzzy_sp_to_kn_limit)) {
          ...
          space = FALSE;
#ifndef GRAPHICS_DISABLED
        mark_gap(blob_box, 1, prev_gap, prev_blob_box.width(), current_gap,
                 next_blob_box.width(), next_gap);
#endif
      }
      ...
      a narrow blob then this gap is a kern as well */
        ...
        if ((tosp_all_flips_fuzzy) || (current_gap > fuzzy_sp_to_kn_limit)) {
          ...
          space = FALSE;
#ifndef GRAPHICS_DISABLED
        mark_gap(blob_box, 2, prev_gap, prev_blob_box.width(), current_gap,
                 next_blob_box.width(), next_gap);
#endif
        ...
        if ((tosp_all_flips_fuzzy) || (current_gap > fuzzy_sp_to_kn_limit)) {
          ...
          space = FALSE;
#ifndef GRAPHICS_DISABLED
        mark_gap(blob_box, 3, prev_gap, prev_blob_box.width(), current_gap,
                 next_blob_box.width(), next_gap);
#endif
      } else if ((next_blob_box.width() > 0) &&
                 narrow_blob(row, next_blob_box) &&
        ...
        if ((tosp_all_flips_fuzzy) || (current_gap > fuzzy_sp_to_kn_limit)) {
          ...
          space = FALSE;
#ifndef GRAPHICS_DISABLED
        mark_gap(blob_box, 4, prev_gap, prev_blob_box.width(), current_gap,
                 next_blob_box.width(), next_gap);
#endif
      } else if ((((next_blob_box.width() > 0) &&
                   narrow_blob(row, next_blob_box)) ||
        ...
        fuzzy_sp = TRUE;
#ifndef GRAPHICS_DISABLED
        mark_gap(blob_box, 6, prev_gap, prev_blob_box.width(), current_gap,
                 next_blob_box.width(), next_gap);
#endif
      }
      ...
      */
      if ((prev_blob_box.width() > 0) && (next_blob_box.width() > 0) &&
          (current_gap >= tosp_kern_gap_factor1 * MAX(prev_gap, next_gap)) &&
        ...
        */
        if ((tosp_flip_fuzz_kn_to_sp) &&
          ...
          fuzzy_non = TRUE;
#ifndef GRAPHICS_DISABLED
        mark_gap(blob_box, 7, prev_gap, prev_blob_box.width(), current_gap,
                 next_blob_box.width(), next_gap);
#endif
        ...
        fuzzy_non = TRUE;
#ifndef GRAPHICS_DISABLED
        mark_gap(blob_box, 8, prev_gap, prev_blob_box.width(), current_gap,
                 next_blob_box.width(), next_gap);
#endif
        ...
        fuzzy_non = TRUE;
#ifndef GRAPHICS_DISABLED
        mark_gap(blob_box, 9, prev_gap, prev_blob_box.width(), current_gap,
                 next_blob_box.width(), next_gap);
#endif
      }
    }
...
void Textord::peek_at_next_gap(TO_ROW *row, BLOBNBOX_IT box_it,
                               TBOX &next_blob_box, inT16 &next_gap,
...
void Textord::mark_gap(TBOX blob,  // blob following gap
                       ...
                       inT16 prev_gap, inT16 prev_blob_width, inT16 current_gap,
                       inT16 next_blob_width, inT16 next_gap) {
...
BOOL8 Textord::ignore_big_gap(TO_ROW *row, inT32 row_length, GAPMAP *gapmap,
 ...
 *************************************************************************/
TBOX Textord::reduced_box_for_blob(BLOBNBOX *blob, TO_ROW *row,
