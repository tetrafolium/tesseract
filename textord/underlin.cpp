  ...
  for (under_it.mark_cycle_pt(); !under_it.cycled_list(); under_it.forward()) {
    ...
    for (cell_it.mark_cycle_pt(); !cell_it.cycled_list(); cell_it.forward()) {
      ...
      if (cell_it.data()->y() - chop_coord > textord_fp_chop_error + 1) {
        split_to_blob(u_line, chop_coord, textord_fp_chop_error + 0.5,
...
TO_ROW *most_overlapping_row( // find best row
  ...
  if (bestover < 0 && row->baseline.y(x) + row->xheight + row->ascrise -
                              blob->bounding_box().bottom() >
                          bestover)
    best_row = row;
  return best_row;
...
void find_underlined_blobs(    // get chop points
  ...
  for (out_it.mark_cycle_pt(); !out_it.cycled_list(); out_it.forward()) {
    vertical_cunderline_projection(out_it.data(), baseline, xheight,
                                   baseline_offset, &lower_proj, &middle_proj,
                                   &upper_proj);
  }

  for (x = blob_box.left(); x < blob_box.right(); x++) {
    if (middle_proj.pile_count(x) > 0) {
      for (y = x + 1; y < blob_box.right() && middle_proj.pile_count(y) > 0;
           y++)
        ;
...
void vertical_cunderline_projection( // project outlines
  ...
  for (stepindex = 0; stepindex < length; stepindex++) {
    ...
    if (step.x() > 0) {
      ...
      upper_y =
          (inT16)floor(baseline->y(pos.x()) + baseline_offset + xheight + 0.5);
      if (pos.y() >= lower_y) {
      ...
      upper_y = (inT16)floor(baseline->y(pos.x() - 1) + baseline_offset +
                             xheight + 0.5);
      if (pos.y() >= lower_y) {
