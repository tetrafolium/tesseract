...
public:
  ...
  const ICOORD &bleft() const { return bleft_; }
  const ICOORD &tright() const { return tright_; }
...
public:
  GridSearch(BBGrid<BBC, BBC_CLIST, BBC_C_IT> *grid)
      : grid_(grid), unique_mode_(false), previous_return_(NULL),
        next_return_(NULL) {}

  // Get the grid x, y coords of the most recently returned BBC.
  int GridX() const { return x_; }
  int GridY() const { return y_; }
  ...
  void SetUniqueMode(bool mode) { unique_mode_ = mode; }
...
template <class BBC> int SortRightToLeft(const void *void1, const void *void2) {
  // The void*s are actually doubly indirected, so get rid of one level.
  const BBC *p1 = *static_cast<const BBC *const *>(void1);
  const BBC *p2 = *static_cast<const BBC *const *>(void2);
...
template <class BBC> int SortByBoxBottom(const void *void1, const void *void2) {
  // The void*s are actually doubly indirected, so get rid of one level.
  const BBC *p1 = *static_cast<const BBC *const *>(void1);
  const BBC *p2 = *static_cast<const BBC *const *>(void2);
...
template <class BBC, class BBC_CLIST, class BBC_C_IT>
BBGrid<BBC, BBC_CLIST, BBC_C_IT>::BBGrid(int gridsize, const ICOORD &bleft,
                                         const ICOORD &tright)
    : grid_(NULL) {
  ...
  for (int y = 0; y < height; ++y) {
    ...
    for (int x = 0; x < width; ++x) {
      if (GET_DATA_BIT(data, x)) {
        grid_[(bottom + y) * gridwidth_ + x + left].add_sorted(
            SortByBoxLeft<BBC>, true, bbox);
      }
    }
  }
}
...
template <class BBC, class BBC_CLIST, class BBC_C_IT>
ScrollView *
BBGrid<BBC, BBC_CLIST, BBC_C_IT>::MakeWindow(int x, int y,
                                             const char *window_name) {
  ScrollView *tab_win = NULL;
#ifndef GRAPHICS_DISABLED
  tab_win = new ScrollView(window_name, x, y, tright_.x() - bleft_.x(),
                           tright_.y() - bleft_.y(), tright_.x() - bleft_.x(),
                           tright_.y() - bleft_.y(), true);
...
BBC *GridSearch<BBC, BBC_CLIST, BBC_C_IT>::NextRadSearch() {
  do {
    while (it_.cycled_list()) {
      ...
      if (x_ >= 0 && x_ < grid_->gridwidth_ && y_ >= 0 &&
          y_ < grid_->gridheight_)
        SetIterator();
    }
    CommonNext();
...
// for a box that vertically overlaps the given vertical line segment.
template <class BBC, class BBC_CLIST, class BBC_C_IT>
void GridSearch<BBC, BBC_CLIST, BBC_C_IT>::StartSideSearch(int x, int ymin,
                                                           int ymax) {
