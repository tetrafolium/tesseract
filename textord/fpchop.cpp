...
EXTERN INT_VAR(textord_fp_chop_error, 2, "Max allowed bending of chop cells");
...
ROW *fixed_pitch_words( // find lines
  ...
  while (rep_left < cell_it.data()->x()) {
    word = add_repeated_word(&rep_it, rep_left, prev_chop_coord, blanks,
                             row->fixed_pitch, &word_it);
  }
  ...
  for (; !cell_it.cycled_list(); cell_it.forward()) {
    ...
    while (!box_it.empty() &&
      ...
      split_to_blob(box_it.extract(), chop_coord, textord_fp_chop_error + 0.5f,
    ...
    if (!right_coutlines.empty() && left_coutlines.empty())
      split_to_blob(NULL, chop_coord, textord_fp_chop_error + 0.5f,
                    &left_coutlines, &right_coutlines);
    if (!left_coutlines.empty()) {
      ...
      if (rep_left < chop_coord) {
        if (rep_left > prev_chop_coord)
          new_blanks = (uinT8)floor(
              (rep_left - prev_chop_coord) / row->fixed_pitch + 0.5);
        else
          new_blanks = 0;
      } else {
        if (chop_coord > prev_chop_coord)
          new_blanks = (uinT8)floor(
              (chop_coord - prev_chop_coord) / row->fixed_pitch + 0.5);
        else
          new_blanks = 0;
      }
      if (!cblob_it.empty()) {
  ...
  while (!rep_it.empty()) {
    add_repeated_word(&rep_it, rep_left, prev_chop_coord, blanks,
                      row->fixed_pitch, &word_it);
  }
  // at end of line
...
void split_to_blob(                 // split the blob
  ...
  if (!right_coutlines->empty() || real_cblob != NULL)
    fixed_chop_cblob(real_cblob, chop_coord, pitch_error, left_coutlines,
                     right_coutlines);
...
void fixed_chop_cblob(             // split the blob
  ...
  if (!right_it.empty()) {
    while (!right_it.empty()) {
      ...
      right_it.forward();
      fixed_split_coutline(old_right, chop_coord, pitch_error, &left_it,
                           &new_it);
    }
  ...
  if (blob != NULL) {
    ...
    for (blob_it.mark_cycle_pt(); !blob_it.cycled_list(); blob_it.forward())
      fixed_split_coutline(blob_it.extract(), chop_coord, pitch_error, &left_it,
                           &right_it);
    delete blob;
  }
}
...
void fixed_split_coutline( // chop the outline
  ...
  if (srcbox.left() + srcbox.right() <= chop_coord * 2 &&
    ...
    // Needs real chopping.
    if (fixed_chop_coutline(srcline, chop_coord, pitch_error, &left_frags,
                            &right_frags)) {
      for (child_it.mark_cycle_pt(); !child_it.cycled_list();
        ...
        if (srcbox.right() < chop_coord) {
          ...
          if (fixed_chop_coutline(child, chop_coord, 0.0f, &left_frags,
                                  &right_frags)) {
            delete child;
          } else {
            if (srcbox.left() + srcbox.right() <= chop_coord * 2)
...
BOOL8 fixed_chop_coutline(           // chop the outline
  ...
  do {
    ...
    //#endif
    if (!first_frag) {
      save_chop_cfragment(head_index, head_pos, tail_index, tail_pos, srcline,
                          left_frags);
    } else {
  ...
  save_chop_cfragment(head_index, head_pos, first_index, first_pos, srcline,
                      left_frags);
...
C_OUTLINE_FRAG::C_OUTLINE_FRAG( // record fragment
    ...
    inT16 start_index, inT16 end_index) {
  start = start_pt;
  end = end_pt;
  ycoord = start_pt.y();
...
// join pieces
C_OUTLINE_FRAG &
C_OUTLINE_FRAG::operator=(const C_OUTLINE_FRAG &src // fragment to copy
) {
  if (steps != NULL)
    delete[] steps;
