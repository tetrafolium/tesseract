...
void SetBlobStrokeWidth(Pix *pix, BLOBNBOX *blob) {
  ...
  Box *blob_pix_box =
      boxCreate(box.left(), pix_height - box.top(), width, height);
...
void Textord::filter_blobs(ICOORD page_tr,        // top right
  ...
  for (block_it.mark_cycle_pt(); !block_it.cycled_list(); block_it.forward()) {
    block = block_it.data();
    block->line_size =
        filter_noise_blobs(&block->blobs, &block->noise_blobs,
                           &block->small_blobs, &block->large_blobs);
...
BOOL8 Textord::clean_noise_from_row( // remove empties
  ...
  if (textord_test_y > row->base_line(textord_test_x) && textord_show_blobs &&
    ...
    for (blob_it.mark_cycle_pt(); !blob_it.cycled_list(); blob_it.forward()) {
      ...
      if (!word->flag(W_DONT_CHOP)) {
        ...
        for (out_it.mark_cycle_pt(); !out_it.cycled_list(); out_it.forward()) {
          ...
          blob_size = blob_box.width() > blob_box.height() ? blob_box.width()
                                                           : blob_box.height();
  ...
  if (textord_noise_debug) {
    tprintf("Row ending at (%d,%g):", blob_box.right(),
            row->base_line(blob_box.right()));
    tprintf(" R=%g, dc=%d, nc=%d, %s\n",
            norm_count > 0 ? (float)dot_count / norm_count : 9999, dot_count,
            norm_count,
            dot_count > norm_count * textord_noise_normratio && dot_count > 2
                ? "REJECTED"
                : "ACCEPTED");
  }
...
struct BlockGroup {
  ...
  explicit BlockGroup(BLOCK *block)
      : bounding_box(block->bounding_box()), rotation(block->re_rotation()),
        angle(block->re_rotation().angle()), min_xheight(block->x_height()) {
...
void tweak_row_baseline(ROW *row, double blshift_maxshift,
  ...
  if (blob_count == 0)
    return;
  xstarts = (inT32 *)alloc_mem((blob_count + row->baseline.segments + 1) *
                               sizeof(inT32));
  coeffs = (double *)alloc_mem((blob_count + row->baseline.segments) * 3 *
                               sizeof(double));
