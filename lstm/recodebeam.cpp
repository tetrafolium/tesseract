...
RecodeBeamSearch::RecodeBeamSearch(const UnicharCompress &recoder,
    ...
    : recoder_(recoder), beam_size_(0), top_code_(-1), second_code_(-1),
      dict_(dict), space_delimited_(true), is_simple_text_(simple_text),
      null_char_(null_char) {
  if (dict_ != NULL && !dict_->IsSpaceDelimitedLang())
...
void RecodeBeamSearch::DebugBeamPos(const UNICHARSET &unicharset,
  ...
  for (int i = 0; i < heap_size; ++i) {
    ...
    if (node->unichar_id == INVALID_UNICHAR_ID) {
      if (null_best == NULL || null_best->score < node->score)
        null_best = node;
    } else {
