...
void ParseArguments(int *argc, char ***argv) {
  ...
  if (!FLAGS_configfile.empty()) {
    tesseract::ParamUtils::ReadParamsFile(
        FLAGS_configfile.c_str(), tesseract::SET_PARAM_CONSTRAINT_NON_INIT_ONLY,
        ccutil.params());
  }
}
...
MasterTrainer *LoadTrainingData(int argc, const char *const *argv,
  ...
  *file_prefix = "";
  ...
  MasterTrainer *trainer = new MasterTrainer(
      NM_CHAR_ANISOTROPIC, shape_analysis, replication, FLAGS_debug_level);
  IntFeatureSpace fs;
  ...
  iterate(pProtoList) {
    ...
    iterate(list_it) {
      ...
      if (test_p != Prototype && !test_p->Merged) {
        FLOAT32 dist =
            ComputeDistance(Clusterer->SampleSize, Clusterer->ParamDesc,
...
CLASS_STRUCT *SetUpForFloat2Int(const UNICHARSET &unicharset,
  ...
  iterate(LabeledClassList) {
    ...
    Class->Configurations =
        (BIT_VECTOR *)Emalloc(sizeof(BIT_VECTOR) * NumConfigs);
