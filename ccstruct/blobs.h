...
struct EDGEPT {
  EDGEPT()
      : next(NULL), prev(NULL), src_outline(NULL), start_step(0),
        step_count(0) {
    memset(flags, 0, EDGEPTFLAGS * sizeof(flags[0]));
  }
  EDGEPT(const EDGEPT &src) : next(NULL), prev(NULL) { CopyFrom(src); }
...
struct TESSLINE {
  ...
  TESSLINE(const TESSLINE &src) : loop(NULL), next(NULL) { CopyFrom(src); }
  ~TESSLINE() { Clear(); }
...
struct TBLOB {
  TBLOB() : outlines(NULL) {}
  TBLOB(const TBLOB &src) : outlines(NULL) { CopyFrom(src); }
  ~TBLOB() { Clear(); }
