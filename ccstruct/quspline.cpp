...
QSPLINE::QSPLINE(   // constructor
  ...
  for (index = 0; index < segments; index++) {
    ...
    quadratics[index] = QUAD_COEFFS(coeffs[index * 3], coeffs[index * 3 + 1],
                                    coeffs[index * 3 + 2]);
  }
  // right edge
  xcoords[index] = xstarts[index];
...
double QSPLINE::step( // find step functions
  ...
  total = 0;
  while (index1 < index2) {
    total += (double)quadratics[index1 + 1].y((float)xcoords[index1 + 1]);
...
void QSPLINE::extrapolate( // linear extrapolation
  ...
  if (xmax > xcoords[segments]) {
    ...
    quads[dest_segment].c =
        y(xcoords[segments]) - quads[dest_segment].b * xcoords[segments];
    dest_segment++;
...
void QSPLINE::plot(          // draw it
  ...
  for (segment = 0; segment < segments; segment++) {
    increment =
        (double)(xcoords[segment + 1] - xcoords[segment]) / QSPLINE_PRECISION;
    x = xcoords[segment];
...
void QSPLINE::plot(Pix *pix) const {
  ...
  for (segment = 0; segment < segments; segment++) {
    increment = static_cast<double>((xcoords[segment + 1] - xcoords[segment])) /
                QSPLINE_PRECISION;
    x = xcoords[segment];
    for (step = 0; step <= QSPLINE_PRECISION; step++) {
