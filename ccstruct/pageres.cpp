    ...
    while (i < choice->length() && j < best_choice->length()) {
      if (choice->unichar_id(i) != best_choice->unichar_id(j) &&
        ...
        if (debug_level >= 2) {
          ...
          tprintf("i %d j %d Choice->Blob[i].Certainty %.4g"
                  " WorstOtherChoiceCertainty %g Threshold %g\n",
                  i, j, choice->certainty(i), best_choice->certainty(j),
                  threshold);
...
bool WERD_RES::ConditionalBlobMerge(
  ...
  for (int i = 0; i + 1 < best_choice->length(); ++i) {
    ...
    if (new_id != INVALID_UNICHAR_ID &&
        (box_cb == NULL ||
         box_cb->Run(box_word->BlobBox(i), box_word->BlobBox(i + 1)))) {
...
void WERD_RES::fix_quotes() {
  ...
  ConditionalBlobMerge(NewPermanentTessCallback(this, &WERD_RES::BothQuotes),
                       NULL);
}
...
bool WERD_RES::PiecesAllNatural(int start, int count) const {
  ...
  for (int index = start; index < start + count - 1; ++index) {
    if (index >= 0 && index < seam_array.size()) {
      ...
      if (seam != NULL && seam->HasAnySplits())
        return false;
    }
  }
  return true;
...
foundword:
  ...
  if (page_res != NULL && page_res->prev_word_best_choice != NULL) {
    *page_res->prev_word_best_choice = (new_block || prev_word_res == NULL)
                                           ? NULL
